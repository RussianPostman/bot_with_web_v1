...
from handlers.data_handlers import HELLO_TEXT, HELP_TEXT, events_list_handler, \
    get_event_month, month_convert_to_digit, get_event_day, get_event_hour, \
    event_hour_handler, admin_notification_handler, user_booking, print_booking
from handlers.google_tools import update_event, cencel_booking
from handlers.keyboards import generate_bottoms, kb_on_start, kb_empty_booking, \
    kb_booking
...
async def get_event_list(message: types.Message):
    ...
    await  bot.send_message(message.from_user.id, events_list_handler())
...
async def chose_time(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if message.text in validation_list:    
            ...
            data['time'] = event_hour_handler(message.text)
            data['user'] = message.from_user.full_name
        
            update_event(data)
            await bot.send_message(ADMIN_ID, admin_notification_handler(data))
            await state.finish()
...
async def censel_handler(message: types.Message, state: FSMContext):
...
async def semple_censel_handler(message: types.Message):
    await message.reply('Ok', reply_markup=kb_on_start)
...
def register_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(semple_censel_handler, commands=['отмена', 'назад'])
    # машина состояний на бронирование
    dp.register_message_handler(censel_handler, state='*', commands='отмена')
    dp.register_message_handler(censel_handler, Text(equals='отмена', ignore_case=True), state='*')
